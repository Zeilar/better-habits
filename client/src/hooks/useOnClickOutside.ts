...
export function useOnClickOutside<T extends HTMLElement>(callback?: (element?: T) => void, args?: IArgs) {
    ...
    useEffect(() => {
        ...
        function clickHandler(e: MouseEvent): void {
            try {
                ...
                if (!element.contains(e.target as Node) && typeof callback === "function") {
                    callback(element);
                }
            } catch (error) {
                console.error(error);
